...
def register(scheduler, pipeline=None):
    ...
    connection = pipeline if pipeline is not None else scheduler.connection._pipeline()
...
def unregister(scheduler, pipeline=None):
    ...
    if pipeline is None:
        connection = scheduler.connection._pipeline()
    else:
        connection = pipeline
...
def get_keys(queue=None, connection=None):
    """Returnes a list of scheduler keys for a queue"""
